...
class ActionEvent extends Model
{
    ...
    {
        $relations = self::forResourceCreateUpdateRelations($model);

        $changes = array_merge($model->attributesToArray(), $relations['dirty']);
    ...
    public static function forResourceUpdate($user, $model)
    {
        ...
        $original = array_merge($original, $relations['original']);
        $dirty = array_merge($model->getDirty(), $relations['dirty']);
    ...
    private static function forResourceCreateUpdateRelations($parentModel)
    {
        $relations = array(
            'original' => array(),
            'dirty' => array()
        ...
        foreach($parentModel->relations as $relationName => $models) {
            foreach($models as $model) {
                if(!array_key_exists($relationName, $relations['dirty'])) {
                    $relations['dirty'][$relationName] = $model->name;
                }
                else {
                    $relations['dirty'][$relationName] .= ", {$model->name}";
                }
    ...
    public static function forResourceDetach($user, $parent, Collection $models, $pivotClass)
    {
        ...
        return $models->map(function ($model) use ($user, $parent, $pivotClass, $batchId) {
            ...
            $target = Nova::modelInstanceForKey(request()->viaRelationship);
